...
{
	unsigned long long delta = rq_clock(rq) - t->sched_info.last_arrival; // delta = cpu burst time

	rq_sched_info_depart(rq, delta);

	/////////// MY CODE ///////////
	if ( (t->policy == SCHED_NORMAL) && (strcmp(t->comm, "cpu") == 0) ) {  
		// PID 값 
		pid_t pid = task_pid_nr_ns(t, &init_pid_ns); // namespace를 고려해 PID 값 소환 (user PID)
		
		// nice 값 
		int nice = task_nice(t);
		
		// vruntime, vdeadline 값
		unsigned long long vruntime = t->se.vruntime; //vruntime
		unsigned long long vdeadline = t->se.deadline; //vdeadline
		
		// 커널 로그에 출력
		printk(KERN_INFO "[EEVDF] PID: %d,  vruntime: %llu, vdeadline: %llu, Nice: %d, CPU_burst: %llu\n",
		       pid, vruntime, vdeadline, nice, delta);
	}
